...
function EditPond({ match }) {
  ...
  useEffect(() => {
    ...
    function getPondData() {
      fetch(process.env.REACT_APP_API_URL + '/getPond/' + match.params.id)
  ...
  const handleNewPondSubmit = event => {
    ...
    fetch(process.env.REACT_APP_API_URL + '/updatePond/' + match.params.id, {
      method: 'PUT',
      mode: 'cors',
      cache: 'no-cache',
      headers: {
  ...
  const deletePond = (id) => {
    fetch(process.env.REACT_APP_API_URL + '/deletePond/' + id, {
      method: 'DELETE',
      mode: 'cors',
      cache: 'no-cache',
      headers: {
